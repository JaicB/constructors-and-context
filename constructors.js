...
function Employee(name, email, hireDate) {
  ...
  */
  ...
  Movie.prototype.changeRating = function (number) {
    return (this.rating + this.number) / 2
}
  ...
  User.prototype.addSavedPost = function (id, title, rating) {
    ...
    return this.savedPosts.push(newPosts)
  }
...
User.prototype.removeSavedPost = function (number) {
  for (i = 0; i <= this.savedPosts.length; i++) {
    if (this.savedPosts[i].id === number) {
  ...
  User.prototype.changePostRating = function(id1, rating1) {
    for (i = 0; i <= this.savedPosts.length; i++) {
      if(this.savedPosts[i].id === id1) {
    let yourMom = {id: id1, title: this.savedPosts.title, rating: rating1}
        return this.savedPosts.splice(i,1, yourMom)
      }
    }
  }